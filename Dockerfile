...
ENV JAVA_OPTS -Xmx6g -Xms6g -XX:MaxPermSize=512m -Dfile.encoding=UTF-8
...
RUN mkdir -p /etc/postfix && cat /build/config/postfix/main.cf >> /etc/postfix/main.cf && \
  ...
  $RSYNC_MOVE /build/scripts/dspace_cron /etc/cron.d/dspace_cron && chmod 644 /etc/cron.d/dspace_cron && \
